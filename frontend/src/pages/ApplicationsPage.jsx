...
export function ApplicationsPage() {
  ...
  const resource = useResource(async () => {
    if (!organizationId) return { applications: [], environments: [], servers: [] }
    const [applications, environments, servers] = await Promise.all([api(organizationPath(organizationId, '/applications')), loadEnvironmentOptions(organizationId), api(organizationPath(organizationId, '/servers'))])
    return { applications: applications.applications, environments, servers: servers.servers }
  }, [organizationId])
  ...
  return <Page title="Applications" description="Deployable workloads, runtime configuration, and current Docker state." actions={organizationId && <CreateButton onClick={() => setCreateOpen(true)}>Create application</CreateButton>}>
    ...
    <Section title="Registered applications">
      <table><thead><tr><th>Name</th><th>Environment</th><th>Target</th><th>Source</th><th>Image</th><th>Port binding</th><th>Created</th><th>Actions</th></tr></thead><tbody>
        ...
        </tr>) : <tr><td colSpan="8"><EmptyState title="No applications yet">Create an environment before registering an application.</EmptyState></td></tr>}
      </tbody></table>
...
function ApplicationForm({ open, onClose, environments, servers, onSaved }) {
  ...
  return <Dialog title="Create application" open={open} onClose={onClose}>{environments.length ? <form className="form-stack" onSubmit={submit}>
    ...
    <Field label="Target server" hint="Select an enrolled Agent host, or leave local to use the explicitly enabled control-plane Docker provider."><select name="serverId" defaultValue=""><option value="">Control plane (local)</option>{servers.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.connectionStatus}</option>)}</select></Field>
