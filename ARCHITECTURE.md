...
| `providers/runtime` | Typed deploy/lifecycle/inspection/status/log contract and local Docker adapter | `Provider`, `DockerRuntimeProvider` | Docker CLI only in adapter | Future typed agent transport | Every destructive action verifies complete Silicon ownership labels |
