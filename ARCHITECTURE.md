...
| `deployments` | State vocabulary and allowed transitions; historical deployment rows/events | `ValidateTransition`, deployment endpoints | PostgreSQL, audit | Job runner and runtime provider | Invalid transitions fail; transition unit tests |
| `providers/runtime` | Contract for workload lifecycle and runtime logs | `Provider` | None in core | Future Docker adapter, later Kubernetes adapter | No implementation in Milestone 1 prevents accidental execution |
