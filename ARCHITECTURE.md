...
| `execution` | Opt-in exact-revision GitHub archive build and local Docker container replacement | `LocalDockerExecutor` | Git provider, Docker CLI | Future remote agent executor | Disabled by default; archive paths/links are constrained; no implicit port publication |
| `providers/runtime` | General workload lifecycle and runtime-log contract | `Provider` | None in core | Future typed local/agent adapters | The narrow GitHub Docker executor does not bypass this future lifecycle boundary |
...
| `providers/secrets` | Secret write/resolve/delete boundary | `Provider` | Future encryption/key management | Local encrypted store, Vault/cloud stores | No plaintext API or logging path is present |
